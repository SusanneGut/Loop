...
namespace Loop.Models
{
    public class MembersService
    {
        ...
        public async Task<MemberActivityVM> GetTotalTime(int id)
        {
            return await context
                .Timestamp
                .Where(o => o.ActivityId == id)
                .Select(s => new MemberActivityVM
                {
                    TotalTime = TimeSpan.Parse(s.TotalTime)
                })
                .SingleOrDefaultAsync();
        }
        ...
        {
            var selectedList = context.Timestamp.Where(o => o.ActivityId == id);
            string totalTime = TimeSpan.Zero.ToString();

            int listOfTimesLength = selectedList.Count();

            if(listOfTimesLength > 1)
            {
                Timestamp secondToLastRow = selectedList.OrderByDescending(p => p.Id).Skip(1).Take(1).Last();
                totalTime = selectedList.Last().TotalTime;
            }

            if(listOfTimesLength < 1 || selectedList.Last().Stop != null)
            {
                await context
                    .Timestamp
        ...
        {
            bool listIsEmpty = !context.Timestamp.Where(o => o.ActivityId == id).Any();
